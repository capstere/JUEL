(() => {
  ...
  const doorTopLink = $("doorTopLink");
  const replayBtn = $("replayBtn");
  const replayBtnWin = $("replayBtnWin");
  ...
  function showHint(){
    if (state.unlocked){
      bubble("Redan klart.", 1400);
      return;
    }
    if (state.secretStep === 0){
      bubble("Börja med ⏰. Punkt.", 1500);
    ...
    // lite torr humor ibland
    if (Math.random() < 0.20){
      const extras = ["Testa något annat.", "Nej.", "Du är nära. Tyvärr.", "Fortsätt bara."];
      toast(extras[(Math.random()*extras.length)|0], 1400);
    }
    sfxHint();
  }
...
helpBtn?.addEventListener("click", ()=> helpModal?.classList.remove("hidden"));
  ...
  function showDoorEntryPoints(){
    // show topbar shortcuts after first win (or if already unlocked)
    if (doorTopLink) doorTopLink.classList.remove("hidden");
    if (replayBtn) replayBtn.classList.remove("hidden");
  }

  function tryOpenWonder(){
    wonder?.classList.remove("hidden");
    showDoorEntryPoints();
    document.body.classList.add("win");
    // confetti should not auto-start here; just show modal again
    updateMood();
  }
  ...
  replayBtnWin?.addEventListener("click", onReplay);
  doorTopLink?.addEventListener("click", ()=>{ unlockAch("door"); });
  ...
  hintBtn?.addEventListener("click", ()=>{
    if (state.unlocked){
      // after win: show win box again (no reload needed)
      tryOpenWonder();
      return;
    }
    showHint();
...
try {
  ...
  if (v === null){
    // Default: ljud PÅ (men startar först vid första tryck = Safari-kompatibelt)
    audioEnabled = true;
    pendingAutoStart = true;
    localStorage.setItem("jesper_audio_enabled", "1");
  } else {
    audioEnabled = v === "1";
...
setSoundBtnLabel();
updateAchUI();
if (audioEnabled) setTimeout(()=>toast("Ljud: på. (Safari kräver första tryck.)", 2200), 650);
  ...
  canvas.addEventListener("pointerdown", (e)=>{
    ...
    if ((state.boopCd||0) <= 0){
      ...
      if (w.x >= bx && w.x <= bx + 16 && w.y >= by && w.y <= by + 16){
        ...
        saveMeta();
        updateAchUI();
        toast(`Dutt +1 (tot: ${meta.boops})`, 1100);
        sfxBoop();
...
if (!("PointerEvent" in window)){
  ...
  function resetGame(){
    // reset game state (keep achievements)
    ...
    // reset ornaments (positions)
    ornaments[0].x = 300; ornaments[0].vx = 0;
    ornaments[1].x = 360; ornaments[1].vx = 0;
    ornaments[2].x = 520; ornaments[2].vx = 0;
    ...
    // allow replay even if previously unlocked
    try { localStorage.removeItem("jesper_unlocked"); } catch (e) {}
    toast("Okej. Ny runda.", 1400);
  }
...
function drawFramePlaceholder(){
  ...
  // ---------- Layout vars (iOS Safari 100vh + dynamic topbar) ----------
