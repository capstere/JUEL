(() => {
  ...
  function tickCountdown(){
    ...
    if (diff <= 0){ countdownEl.textContent = "Kaffe. ☕"; return; }
  ...
  const doorShortcut = $("doorShortcut");
  const resetTopBtn = $("resetTopBtn");
  const resetBtn = $("resetBtn");
  const frameOverlay = $("frameOverlay");
  const frameGif = $("frameGif");
  const boopHotspot = $("boopHotspot");
  ...
  function resetGame(){
    // close modals
    try { helpModal?.classList.add("hidden"); } catch(e){}
    try { achModal?.classList.add("hidden"); } catch(e){}
    closeWonder();
    ...
    if (state.frameFx){
      state.frameFx.glitchT = 0;
      state.frameFx.freezeT = 0;
      state.frameFx.winkT = 0;
      state.frameFx.nextAt = 0;
      state.frameFx.freezeCanvas = null;
    }
    ...
    // reset run timer (speedrunner can be re-attempted)
    runStartMs = null;
    runEndMs = null;

    // show shortcuts if user has ever won (door/reset)
    updateMood();
    updateShortcuts();

    toast("🔁 Spela om HAHA.", 1400);
  }
  ...
  hintBtn?.addEventListener("click", ()=>{
    ...
    if (state.unlocked){
      // allow user to access door again OR replay
      wonder?.classList.remove("hidden");
      document.body.classList.add("win");
      bubble("Du kan öppna 🚪 eller spela om 🔁", 1700);
      return;
    }
    if (state.secretStep === 0){
      bubble("Börja med med tiden.", 1700);
    } else if (state.secretStep === 1){
      bubble("Candy", 1700);
    } else if (state.secretStep === 2){
      bubble("Stjeirnrnra.", 1700);
    } else if (state.secretStep >= 3){
      bubble("🪑 Sätt dig ner. Nu.", 1700);
    } else {
  ...
  doorLink?.addEventListener("click", ()=>{
    unlockAch("door");
    toast("🚪 Portalen öppnad!", 1200);
  });

  doorShortcut?.addEventListener("click", ()=>{
    unlockAch("door");
    toast("🚪 Portalen öppnad!", 1200);
  });

  boopHotspot?.addEventListener("click", (e)=>{
    e.preventDefault();
    e.stopPropagation();
    markRunStart();
    // trigger same boop logic as secret pixel (but DOM-based so it works on all mobiles)
    state.boopCd = 0.25;
...
try {
  const v = localStorage.getItem("jesper_audio_enabled");
  if (v === "0") audioEnabled = false;
  if (v === "1") audioEnabled = true;
  pendingAutoStart = audioEnabled;
} catch (e) {
  // iOS/Safari can block storage in some modes; keep defaults
  pendingAutoStart = audioEnabled;
}
setSoundBtnLabel();
updateAchUI();
...
// Om användaren byter app/flik: pausa musiken så iOS inte blir grinig.
  ...
  function resize(){
    ...
    // keep GIF overlay aligned on mobile when viewport changes
    try { positionFrameOverlay(); } catch (e) {}
  }
  ...
  function positionFrameOverlay(){
    ...
    // small inset so it sits inside the drawn frame border
    const inset = 6;
    const x = (f.x + inset) * view.s + view.ox;
    const y = (f.y + inset) * view.s + view.oy;
    const w = Math.max(2, (f.w - inset*2) * view.s);
    const h = Math.max(2, (f.h - inset*2) * view.s);

    frameOverlay.style.transform = `translate(${x}px, ${y}px)`;
    frameOverlay.style.width = `${w}px`;
    frameOverlay.style.height = `${h}px`;
  }
  ...
  function updateShortcuts(){
    const show = !!(state.unlocked || (meta.wins||0) > 0);
    if (doorShortcut) doorShortcut.classList.toggle("hidden", !show);
    if (resetTopBtn) resetTopBtn.classList.toggle("hidden", !show);
  }
  updateShortcuts();
  ...
  // ---------- Legacy unlock flag (localStorage) ----------
// Older builds used `jesper_unlocked=1` which permanently locked the game in "won" state.
// We now treat it as "won at least once" (for showing the door shortcut), but allow replay.
try {
  const legacy = localStorage.getItem("jesper_unlocked") === "1";
  if (legacy && (!meta.wins || meta.wins < 1)){
    meta.wins = 1;
    saveMeta();
  }
  // remove legacy key to avoid future lock-in
  try { localStorage.removeItem("jesper_unlocked"); } catch(e){}
} catch (e) {}
  ...
  canvas.addEventListener("pointerdown", (e)=>{
    ...
    if ((state.boopCd||0) <= 0){
      ...
      const bx = f.x + f.w - 20;
      const by = f.y + f.h - 20;
      if (w.x >= bx && w.x <= bx + 16 && w.y >= by && w.y <= by + 16){
        state.boopCd = 0.25;
        meta.boops = (meta.boops||0) + 1;
...
if (!("PointerEvent" in window)){
  ...
  function advanceSecret(id){
    ...
    if (Math.random() < 0.25){
      const lines = ["Calculated.", "Lett som en plett.", "Det går bra.", "Enkelt."];
  ...
  function unlock(){
    ...
    bubble("… yay du klarade det typ.✨", 1700);
  ...
  function doDance(){
    if (!state.unlocked){
      bubble("Du måste förtjäna dans.", 1400);
      sfxNopeSoft();
      return;
    }
    markRunStart();
